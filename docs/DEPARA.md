...
| `get_leads(p_busca, p_perfil, p_limit)` (RPC, SECURITY DEFINER, staff-only) | `profiles`, `leads`, `searches`, `video_views` | `admin-leads` | **[Extensão do doc]** Lista de pessoas com comportamento já resumido, em um round-trip. Busca textual cobre nome, e-mail, WhatsApp e o texto das dores pesquisadas. |
