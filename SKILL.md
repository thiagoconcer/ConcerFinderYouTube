...
> **O que é este arquivo:** o manual que a IA de desenvolvimento (Lovable) deve seguir para construir o **ConcerFinder** — a plataforma de busca semântica nos vídeos do canal do Thiago Concer. Não é conselho genérico: são as regras **deste** projeto. Leia antes de escrever qualquer linha e volte a ele a cada etapa.
>
...
- **Caminho de build:** **Lovable + Supabase** — React + Tailwind + shadcn/ui, com integração nativa ao Supabase e deploy com preview. Escolhido porque o time opera com ferramentas no-code/low-code (N8N, ActiveCampaign, Sellflux) e não mantém stack de código próprio; Lovable entrega do zero ao ar pelo caminho mais rápido. **Não** troque por Next.js, Vue ou Angular.
- **Nomenclatura:**
...
- **Autenticação de escrita sensível:** funções que gravam em `leads`, `videos`, `video_segments`, `search_results` e `ingestion_runs` rodam com **`service_role`** — chave **jamais** exposta no frontend Lovable, só dentro de Edge Functions/Cron.
...
- ❌ **Não** expor `service_role` (nem chaves de YouTube/Apify, ActiveCampaign, WhatsApp) no cliente Lovable — só dentro de Edge Functions/Cron.
...
- ❌ **Não** trocar o caminho de build (nada de Next.js/Vue/Angular) — o frontend é React gerado pelo Lovable.
